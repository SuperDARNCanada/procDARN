...
fn fit_rawacf_record(record: &RawacfRecord) -> Result<FitacfRecord> {
    ...
    let fitacf = determinations(record, range_list, noise_power);
...
fn acf_cutoff_power(rec: &RawacfRecord) -> f32 {
    let sorted_power_levels = rec.lag_zero_power.data.clone();
...
fn determinations(rec: &RawacfRecord, ranges: Vec<RangeNode>, noise_power: f32) -> Result<FitacfRecord> {
    let hdw = HdwInfo::new(rec.station_id);
    let range_list: Vec<i16> = ranges.iter().map(|r| r.range_num as i16).collect();
    let lag_0_power_db: Vec<f32> = rec.lag_zero_power.data
        .iter()
        .map(|p| {
    ...
    if range_list.len() == 0 {
        Ok(FitacfRecord {
            ...
            velocity_noise: 0.0,
            tdiff: None,
            pulse_table: rec.pulse_table,
            lag_table: rec.lag_table,
        ...
        let num_lags: Vec<i16> = ranges.iter().map(|r| r.powers.ln_power.len() as i16).collect();
        ...
        let power_linear: Vec<f32> = ranges
            .iter()
            .map(|r| {
                10.0 * r.lin_pwr_fit.expect("Unable to make fitacf without linear fitted power")
                    .intercept as f32 / (10.0 as f32).log10() - noise_db
            })
            .collect();
        let power_linear_error: Vec<f32> = ranges
            .iter()
            .map(|r| {
                10.0 * (r.lin_pwr_fit_err.expect("Unable to make fitacf without linear fitted power error")
                    .variance_intercept as f32).sqrt() / (10.0 as f32).log10()
            })
            .collect();
        let power_quadratic: Vec<f32> = ranges
            .iter()
            .map(|r| {
                10.0 * (r.quad_pwr_fit.expect("Unable to make fitacf without quadratic fitted power")
                    .intercept as f32) / (10.0 as f32).log10() - noise_db
            })
            .collect();
        let power_quadratic_error: Vec<f32> = ranges
            .iter()
            .map(|r| {
                10.0 * (r.quad_pwr_fit_err.expect("Unable to make fitacf without quadratic fitted power error")
                    .variance_intercept as f32).sqrt() / (10.0 as f32).log10()
            })
            .collect();
        let velocity_conversion: f32 = 299792458.0 * hdw.vel_sign / (4.0 * PI_f32 * rec.tx_freq as f32 * 1000.0);
        let velocity: Vec<f32> = ranges
            .iter()
            .map(|r| {
                (r.phase_fit.expect("Unable to make fitacf without fitted velocity")
                    .slope as f32) * velocity_conversion
            })
            .collect();
        let velocity_error: Vec<f32> = ranges
            ...
            .map(|r| {
                (r.phase_fit.expect("Unable to make fitacf without fitted velocity")
                    .variance_slope as f32).sqrt() * velocity_conversion
            })
            .collect();
        let width_conversion: f32 = 299792458.0 * 2.0 / (4.0 * PI_f32 * rec.tx_freq as f32 * 1000.0);
        let spectral_width_linear: Vec<f32> = ranges
            .iter()
            .map(|r| {
                (r.lin_pwr_fit.expect("Unable to make fitacf spectral width without fitted power")
                    .slope as f32).abs() * width_conversion
            })
            .collect();
        let spectral_width_linear_error: Vec<f32> = ranges
            .iter()
            .map(|r| {
                (r.lin_pwr_fit_err.expect("Unable to make fitacf spectral width error without fitted power error")
                    .variance_slope as f32).sqrt() * width_conversion
            })
            .collect();
        let quadratic_width_conversion: f32 = 299792458.0 * (2.0 as f32).sqrt() / (PI_f32 * rec.tx_freq as f32 * 1000.0);
        let spectral_width_quadratic: Vec<f32> = ranges
            .iter()
            .map(|r| {
                (r.quad_pwr_fit.expect("Unable to make fitacf quadratic spectral width without fitted power")
                    .slope as f32).abs() * quadratic_width_conversion
            })
            .collect();
        let spectral_width_quadratic_error: Vec<f32> = ranges
            .iter()
            .map(|r| {
                (r.quad_pwr_fit_err.expect("Unable to make fitacf quadratic spectral width error without fitted power error")
                    .variance_slope as f32).sqrt() * quadratic_width_conversion /
                    ((r.quad_pwr_fit.expect("Unable to make fitacf quadratic spectral width error without fitted power error")
                        .slope as f32).abs().sqrt() * 2.0)
            })
            .collect();
        let std_dev_linear: Vec<f32> = ranges
            .iter()
            .map(|r| r.lin_pwr_fit.expect("Unable to make fitacf linear std deviation without fitted power")
                .chi_squared as f32)
            .collect();
        let std_dev_quadratic: Vec<f32> = ranges
            .iter()
            .map(|r| r.quad_pwr_fit.expect("Unable to make fitacf quadratic std deviation without fitted power")
                .chi_squared as f32)
            .collect();
        let std_dev_phi: Vec<f32> = ranges
            .iter()
            .map(|r| r.phase_fit.expect("Unable to make fitacf phi std deviation")
                .chi_squared as f32)
            .collect();
        let groundscatter_flag: Vec<i8> = zip(velocity.iter(), spectral_width_linear.iter())
            .map(|(v, w)| {
                if v.abs() - (V_MAX - w * (V_MAX / W_MAX)) < 1.0 { 1 } else { 0 }
            })
            .collect();
        let xcf_phi0: Vec<f32> = ranges
            .iter()
            .map(|r| {
                let xcfs = rec.xcfs.expect("Unable to make fitacf xcf_phi0").data;
                xcfs[r.range_idx * rec.num_lags as usize * 2 + 1].atan2(xcfs[r.range_idx * rec.num_lags as usize * 2])
                    * hdw.phase_sign
            })
            .collect();
        let xcf_phi0_err: Vec<f32> = ranges
            .iter()
            .map(|r| (r.elev_fit.expect("Unable to make fitacf xcf_phi0_err")
                .chi_squared as f32).sqrt())
            .collect();
        let xcf_phi_std_dev: Vec<f32> = ranges
            .iter()
            .map(|r| r.elev_fit.expect("Unable to make fitacf xcf_phi_std_dev")
                .chi_squared as f32)
            .collect();
        let (elevation_low, elevation_normal, elevation_high) = calculate_elevation(&ranges, rec, &xcf_phi0, hdw);
        ...
        Ok(FitacfRecord {
            ...
            elevation: None,
            elevation_fitted: Some(convert_to_dmapvec(elevation_normal)),
            elevation_error: None,
...
fn calculate_elevation(ranges: &Vec<RangeNode>, rec: &RawacfRecord, xcf_phi0: &Vec<f32>, hdw: &HdwInfo) -> (Vec<f32>, Vec<f32>, Vec<f32>) {
    let x = hdw.intf_offset_x;
    let y = hdw.intf_offset_y;
    let z = hdw.intf_offset_z;

    let array_separation: f32 = (x*x + y*y + z*z).sqrt();
    let elevation_corr = (z / array_separation).asin();

    let mut phi_sign: f32;
    if y > 0.0 {
        phi_sign = 1.0;
    } else {
        phi_sign = -1.0;
    ...
    let phi_0 = (hdw.beam_separation * (rec.beam_num as f32 - azimuth_offset)
        * PI as f32 / 180.0).cos();
    let wave_num = 2.0 * PI_f32 * rec.tx_freq as f32 * 1000.0 / 299792458.0;
    let cable_offset = -2.0 * PI_f32 * rec.tx_freq as f32 * 1000.0 * hdw.tdiff * 1.0e-6;
    let phase_diff_max = phi_sign * wave_num * array_separation * phi_0 + cable_offset;
    let psi: Vec<f32> = ranges
        .iter()
        .map(|r| {
            let x = r.elev_fit.expect("Unable to find elevation without fitted elevation").intercept as f32;
            let y = x + 2.0 * PI_f32 * ((phase_diff_max - x) / (2.0 * PI_f32)).floor() - cable_offset;
            if phi_sign < 0.0 {
                y += 2.0 * PI_f32;
            }
            y
        ...
        .collect();
    let psi_kd: Vec<f32> = psi
        .iter()
        .map(|p| p / (wave_num * array_separation))
        .collect();
    let theta: Vec<f32> = psi_kd
        .iter()
        .map(|p| phi_0 * phi_0 - p * p)
        .collect();
    ...
    let errors: Vec<f32> = ranges
        .iter()
        .map(|r| r.elev_fit.expect("Unable to calculate elevation errors")
    ...
    psi = xcf_phi0
        .iter()
        .map(|&x| {
            let y = x as f32 + 2.0 * PI_f32 * ((phase_diff_max - x as f32) / (2.0 * PI_f32)).floor() - cable_offset;
            if phi_sign < 0.0 {
                y += 2.0 * PI_f32;
            }
            y
